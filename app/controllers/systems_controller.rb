...
class SystemsController < ApplicationController
  ...
  def create
    inserts, deletes = PolicySystem.bulk_assign(new_policy_systems, old_policy_systems)
  ...
  def new_policy_system
    @new_policy_system ||= begin
      ...
      left = pundit_scope(Policy).where.not(id: right.policies.select(:id))
                                 .find(permitted_params[:policy_id])

      PolicySystem.new(policy: left, system: right)
    end
  end
  ...
  def systems_with_sibling_policies
    PolicySystem.joins(policy: :profile)
  ...
  def policy
    @policy ||= pundit_scope(Policy).find(permitted_params[:policy_id])
  end

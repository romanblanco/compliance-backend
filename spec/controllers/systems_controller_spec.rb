...
describe SystemsController do
  ...
  let(:attributes) do
    {
      ...
      last_check_in: -> { last_check_in.as_json },
  ...
  context '/policies/:id/systems' do
    ...
    let(:parent) do
      FactoryBot.create(:policy, :for_tailoring, account: current_user.account, supports_minors: [0, 1, 2, 8])
    end
    ...
    RSpec.shared_examples 'assign to policy' do
      ...
      context 'policy created from a no longer supported profile' do
        let(:parent) { FactoryBot.create(:policy, account: current_user.account) }

        before { FactoryBot.create(:profile, ref_id: parent.profile.ref_id, supports_minors: [0, 1, 2, 8]) }
        ...
        context 'mismatch in supported rules' do
          before { FactoryBot.create(:rule, profile_id: parent.profile_id) }
    ...
    RSpec.shared_examples 'unassign from policy' do
      ...
      context 'multiple policies linked to the system' do
        ...
        before { FactoryBot.create(:policy_system, policy: policy, system: item) }
    ...
    describe 'PATCH update' do
      ...
      context 'via CERT_AUTH' do
        ...
        let(:current_user) { FactoryBot.create(:user, :with_cert_auth, system_owner_id: owner_id) }

        include_examples 'assign to policy'

        context 'system owner_id mismatch' do
          let(:current_user) { FactoryBot.create(:user, :with_cert_auth, system_owner_id: Faker::Internet.uuid) }

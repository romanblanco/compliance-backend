...
describe TestResultsController do
  ...
  context '/reports/:id/test_results' do
    let(:parent) do
      ...
      (1..4).to_a.each do |v|
        sg = FactoryBot.create(:security_guide, os_major_version: report.os_major_version, version: "0.0.#{v}")
        FactoryBot.create(:profile, ref_id: report.ref_id, security_guide: sg, supports_minors: [v])
      end
    ...
    describe 'GET index' do
      ...
      let(:items) do
        item_count.times.map do
          FactoryBot.create(
            :test_result, system: FactoryBot.create(:system, **sysparams), report_id: parent.id
          ).reload
        end.sort_by(&:id)
      end

...
describe RuleResultPolicy do
  ...
  let!(:items) do
    test_result.tailoring.rules.map do |rule|
      FactoryBot.create(:rule_result, rule_id: rule.id, test_result_id: test_result.id)
    end
  end
  ...
  context 'org-level access without groups' do
    ...
    it 'allows displaying entities' do
      expect(Pundit.policy_scope(user, RuleResult.joins(:system)).to_set).to eq(items.to_set)
    end
    ...
    it 'authorizes the index and show actions' do
      ...
      RuleResult.where.not(id: items.map(&:id)).find_each do |item|

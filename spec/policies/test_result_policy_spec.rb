...
describe TestResultPolicy do
  ...
  context 'org-level access without groups' do
    ...
    it 'allows displaying entities' do
      expect(Pundit.policy_scope(user, TestResult.joins(:system)).to_set).to eq(items.to_set)
    end
    ...
    it 'authorizes the index and show actions' do
      ...
      TestResult.where.not(id: items.map(&:id)).find_each do |item|
